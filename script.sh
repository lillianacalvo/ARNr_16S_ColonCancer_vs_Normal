...
qiime tools import --type 'SampleData[PairedEndSequencesWithQual --input-path manifiesto.tsv --output-path paired-end-sequences --input-format PairedEndFastqManifestPhred33V2
...
qiime vsearch uchime-denovo --i-table table.qza --i-sequences re--output-dir uchime-dn-out
...
qiime feature-table filter-features --i-table table.qza --m-metadata-file uchime-dn-out/nonchimeras.qza --o-filtered-table uchime-dn-out/table-nonchimeric-wo-borderli
qiime feature-table filter-seqs --i-data rep-seqs.qza --m-metadata-file uchime-dn-out/nonchimeras.qza --o-filtered-data uchime-dn-out/rep-seqs-nonchimeric-wo-border
qiime feature-table summarize

#Análisis filogenético
#El siguiente comando genera un árbol filogenético de los ASVs necesario para el análisis de rarefacción:
qiime phylogeny align-to-tree-mafft-fasttree
  --i-sequences uchime-dn-out/rep-seqs-nonchimeric-wo-borderline
...
--o-visualization uchime-dn-out/table-nonchimeric-wo-borderlin
--m-sample-metadata-file metadatos.tsv
#El siguiente comando genera un archivo con la lista de ASVs
#con su secuencia y un link para buscar en BLAST:
...
#Se descarga la base GSR: no tiene un clasificador pre entrenado
#disponible en qiime2.
wget https://manichanh.vhir.org/gsrdb/GSR-DB_full-16S.tar.gz
tar -xvzf GSR-DB_full-16S.tar.gz
# Train naive-bayes classifier with default parameters
...
qiime feature-classifier classify-sklearn
  ...
  --i-reads rarefied-table.qza
  --o-classification taxonomy_nochimeras.qza
qiime metadata tabulate --m-input-file taxonomy_nochimeras.qza
  --o-visualization taxonomy_nochimeras.qzv
qiime taxa barplot
  --i-table uchime-dn-out/table-nonchimeric-wo-borderline.qza
  --i-taxonomy taxonomy_nochimeras.qza --m-metadata-file metadat
...
--m-metadata-file metadatos.tsv
--o-visualization core-metrics-results-3/faith-pd-group-signif

#Diversidad Beta (variables categóricas)
qiime diversity beta-group-significance
--i-distance-matrix core-metrics-results-3/unweighted_unifrac_
--m-metadata-file metadatos.tsv
--m-metadata-column Description
--o-visualization core-metrics-results-3/unweighted-unifrac-de
--p-pairwise

#Análisis de PCA unweighted
qiime emperor plot
--i-pcoa core-metrics-results-3/unweighted_unifrac_pcoa_result
--m-metadata-file metadatos.tsv
--o-visualization core-metrics-results-3/unweighted-unifrac-em
